...
def train_iteration(batch):
    ...
    # print(seqs[0], "seqs")
    # print(doc_mask[0], "doc_mask")
    # print(selected_y_label[0], "selected_y_label")
    # print(selected_score[0], "selected_score")
    # print(valid_sen_idxs[0], "valid_sen_idxs")
    seqs = seqs.to(device)
    doc_mask = doc_mask.to(device)
    selected_y_label = selected_y_label.to(device)
    #print(valid_sen_idxs, "idx", valid_sen_idxs[0])
    #answer = torch.unique(valid_sen_idxs.clone().detach(), dim=1) #라벨달린거 전부 unique한거
        ...
        if ( validate_every != 0 and  current_batch % validate_every == 0 ) or count == len(train_data_loader) - 1:
            ...
            with torch.no_grad():
                for batch in val_data_loader:
                    tm1, tm2, tm3 = validation_iteration(batch)
                    val_score_list +=tm1 
                    acc += tm2
                    count1  += tm3
            print(acc/count1,"ACCURACY")
